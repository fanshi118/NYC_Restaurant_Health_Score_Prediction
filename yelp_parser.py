# -*- coding: utf-8 -*-
"""
This program is modified based on the sample code provided by Yelp on GitHub:
...
def query_api(term, location):
    ...
    #df_yelp = pd.DataFrame(columns=['zip','rest_score','rest_revcount'])
    df_yelp = pd.DataFrame(columns=['zip','bar_score','bar_revcount'])
    row = 0
    ...
    for zc in zipCodes:
        ...
        for i in range(len(businesses)):
            scoreList.append(businesses[i]['rating'])
            reviewList.append(businesses[i]['review_count'])

        score_mean = np.mean(scoreList)
        review_mean = np.mean(reviewList)
        df_yelp.loc[row,'zip'] = zc
        #df_yelp.loc[row,'rest_score'] = score_mean
        #df_yelp.loc[row,'rest_revcount'] = review_mean
        df_yelp.loc[row,'bar_score'] = score_mean
        df_yelp.loc[row,'bar_revcount'] = review_mean
        row += 1
...
def main():
    ...
    #result.to_csv('data_result/NYC_YelpScore_Restaurants.csv')
    result.to_csv('data_result/NYC_YelpScore_Bars.csv')
